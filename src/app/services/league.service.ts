...
export class LeagueService {
  private readonly ALL_LEAGUES_URL = 'https://www.thesportsdb.com/api/v1/json/3/all_leagues.php';
  private readonly SEASON_BADGES_URL = 'https://www.thesportsdb.com/api/v1/json/3/search_all_seasons.php?badge=1&id=';
  
  private leaguesCache: Observable<League[]> | null = null;
  private seasonBadgesCache = new Map<string, Observable<Season[]>>();

  constructor(private http: HttpClient) {}

  getAllLeagues(): Observable<League[]> {
    if (!this.leaguesCache) {
      this.leaguesCache = this.http.get<LeaguesResponse>(this.ALL_LEAGUES_URL)
        .pipe(
  ...
  getSeasonBadges(leagueId: string): Observable<Season[]> {
    if (!this.seasonBadgesCache.has(leagueId)) {
      const seasonsObservable = this.http.get<SeasonsResponse>(this.SEASON_BADGES_URL + leagueId)
        .pipe(
